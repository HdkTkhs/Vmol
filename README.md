...
* At the end of the 'Vmol' source code, 'Vmol' includes 'ext_routines.f' file (**not** provided in the distribution due to the copyright issues), to incorporates the subroutines and functions
  ...
  Most of these routines can be equivalently replaced by those included in MKL, and the rest can be developed by users. Anyway, the file 'ext_routines.f', which contains these routines, must be prepared by users. 
