...
build a QM/MM simulator[2] in the early stage of the development. The current distribution, however, provides only the original MPI-parallelized Kohn Sham-DFT[4] machinery that has been extracted from the integrated code. Thus, only the core of the 'Vmol' has been placed on GitHub. 
